...
export let makePainter = (info: CaterfoilObjectData): ((l: number, b: GPUCommandEncoder) => void) => {
...
export let callTreeRenderers = (t: number, c: GPUCommandEncoder, el: CaterfoilRenderObject) => {
  if (el == null) return;
  if (el.type === "object") {
    el.renderer(t, c);
  } else {
...
export function paintCaterfoilTree() {
  let device = atomDevice.deref();
  let commandEncoder = device.createCommandEncoder({ label: "caterfoil shared" });

  atomBufferNeedClear.reset(true);
  let tree = atomCaterfoilTree.deref();
